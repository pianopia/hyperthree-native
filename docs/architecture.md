...
| JS-to-native render bridge | `src/bridge.rs` and `js/three-bridge.js` now carry position/normal/UV geometry, PBR material parameters, directional light, `matrixWorld`, and `Points` particles into native PBR/billboard passes | Standard WebGPU device, resource, render/compute pass, and renderer bindings |
