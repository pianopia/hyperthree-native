...
| JS-to-native render bridge | `src/bridge.rs` and `js/three-bridge.js` now carry position/normal/UV geometry, PBR material parameters, directional light, `matrixWorld`, and `Points` particles into native PBR/billboard passes; `src/webgpu.rs` creates native resources, derives pipeline bind-group layouts, executes WebGPU bind-group/pipeline/render/compute/copy commands, supports texture view descriptors, typed and compressed texture uploads across mip levels, GPUBuffer MAP_READ readback, texture/sampler destruction, device-lost signaling, native surface-texture discard, Lost/Outdated surface reconfiguration, and presents a canvas texture through the native surface; the host recreates the device/Renderer/JS session at the next frame boundary after loss | Complete renderer bindings and full canvas configure/unconfigure semantics |
