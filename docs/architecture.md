...
| Asset path | `src/asset.rs` memory-maps project-relative files, inspects glTF/GLB metadata, decodes POSITION/index/UV primitives, EXT_meshopt-compressed views, and base-color images natively; the standard GLTFLoader path verifies raw BC1 KTX2 via KHR_texture_basisu, while the native `basisu` transcoder bridge handles BasisLZ/UASTC KTX2 targets | BasisLZ/UASTC target-format matrix fixtures, Draco textures and full material/animation streaming |
