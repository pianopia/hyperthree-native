...
| JS-to-native render bridge | `src/bridge.rs` and `js/three-bridge.js` pass clear color, camera, built-in primitives, cached arbitrary BufferGeometry, native glTF geometry, material color, keyboard state, and asset metadata into wgpu | WebGPU device, buffer, texture, and command encoder bindings |
