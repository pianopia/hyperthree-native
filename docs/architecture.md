...
| Three.js compatibility seam | `syncThreeScene(scene, camera)` traverses built-in primitives and registers arbitrary Three.js BufferGeometry into the native render list; Three.js 0.179 `WebGPURenderer.renderAsync()` has been smoke-tested with a PBR/light/Points scene on native Metal | complete WebGPU renderer/object/material coverage |
