...
| JS execution outside a browser | `src/js_runtime.rs` runs embedded Boa scripts and ES modules with project-relative/`node_modules`/`exports` resolution, native `performance.now()`, RAF scheduling, window-global aliases, and an opt-in `navigator.gpu` resource binding; the WebGPU shim includes mapped buffer upload, native canvas/DOM types, WGSL diagnostic compatibility, and a narrowly scoped Three.js node-cache `WeakMap` compatibility shim | Embedded V8 isolate and broader Node/Web API compatibility |
...
| JS-to-native render bridge | `src/bridge.rs` and `js/three-bridge.js` now carry position/normal/UV geometry, PBR material parameters, directional light, `matrixWorld`, and `Points` particles into native PBR/billboard passes; `src/webgpu.rs` creates native resources, derives pipeline bind-group layouts, executes WebGPU bind-group/pipeline/render/compute/copy commands, supports texture view descriptors and typed uploads, and presents a canvas texture through the native surface | Complete canvas lifecycle and renderer bindings |
| Three.js compatibility seam | `syncThreeScene(scene, camera)` traverses built-in primitives and registers arbitrary Three.js BufferGeometry into the native render list; Three.js 0.179 `WebGPURenderer.renderAsync()` has been smoke-tested with PBR/light/Points, AnimationMixer transform/morph paths, and SkinnedMesh/Skeleton bone-transform paths on native Metal | complete WebGPU renderer/object/material coverage |
