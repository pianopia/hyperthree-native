...
/// Decode a Draco triangle stream into the interleaved-free attribute model
/// expected by Three.js' DRACOLoader. The decoder returns dequantized values,
/// while the original component values remain representable as f32 and are
/// converted to the typed array requested by the glTF accessor at the JS edge.
pub fn decode_mesh(bytes: &[u8]) -> Result<DracoGeometry> {
    let mesh = Decoder::new()
        .decode_mesh(bytes)
        .map_err(|error| anyhow::anyhow!("Draco decode failed: {error}"))?;

    let mut indices = Vec::with_capacity(mesh.faces.len() * 3);
    for face in &mesh.faces {
        for point in face {
            indices.push(u32::try_from(usize::from(*point)).context("Draco index exceeds u32")?);
        }
    }
...
mod tests {
    ...
    fn decodes_khronos_box_draco_stream() {
        ...
        let geometry = decode_mesh(&bytes).expect("Khronos Box Draco stream should decode");
